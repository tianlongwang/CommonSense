...
import json
with open('readworks_grade3.0.1.json', 'r') as op:
  lines = op.read()
...
XaC = pad_sequences(aCl, maxlen=a_maxlen)
XaD = pad_sequences(aDl, maxlen=a_maxlen)
Y = np.array(ll)
...
aCrnn = Sequential()
aCrnn.add(Embedding(vocab_size, EMBED_HIDDEN_SIZE,
                   input_length=a_maxlen, mask_zero=True, weights=[embedding_weights]))
aCrnn.add(Dropout(0.2))
aCrnn.add(RNN(EMBED_HIDDEN_SIZE, return_sequences=False))
aCrnn.add(RepeatVector(st_maxlen))

aDrnn = Sequential()
aDrnn.add(Embedding(vocab_size, EMBED_HIDDEN_SIZE,
                   input_length=a_maxlen, mask_zero=True, weights=[embedding_weights]))
aDrnn.add(Dropout(0.2))
aDrnn.add(RNN(EMBED_HIDDEN_SIZE, return_sequences=False))
aDrnn.add(RepeatVector(st_maxlen))
...
modelC = Sequential()
modelC.add(Merge([sentrnn, qrnn, aCrnn], mode='sum'))
modelD = Sequential()
modelD.add(Merge([sentrnn, qrnn, aDrnn], mode='sum'))
model = Sequential()
...
print('Training')
model.fit([Xs, Xq, XaA, XaB, XaC, XaD], Y, batch_size=BATCH_SIZE, nb_epoch=EPOCHS, validation_split=0.1)


loss, acc = model.evaluate([Xs, Xq, XaA, XaB, XaC, XaD], Y, batch_size=BATCH_SIZE)
